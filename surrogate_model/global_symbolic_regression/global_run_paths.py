"""Shared dataset-run resolution for global symbolic-regression entry points."""
...
def default_dataset_run() -> str:
    if not RUN_CONFIG_FILE.exists():
        raise FileNotFoundError(f"Global dataset run config not found: {RUN_CONFIG_FILE}")
...
def global_dataset_path(dataset_run: str) -> Path:
    dataset_run = validate_dataset_run(dataset_run)
    return SURROGATE_ROOT / "datasets" / dataset_run / "global_SR" / "Wool_symbolic_global.mat"
...
def resolve_dataset_file(dataset_run: str, dataset_file: Path | None) -> tuple[Path, str]:
    dataset_run = validate_dataset_run(dataset_run)
    resolved_file = global_dataset_path(dataset_run) if dataset_file is None else dataset_file
